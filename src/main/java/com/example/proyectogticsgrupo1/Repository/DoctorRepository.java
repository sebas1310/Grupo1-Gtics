...
public interface DoctorRepository extends JpaRepository<Doctor, Integer>{
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM doctor where idsede = ?1 and idespecialidad = ?2")
    List<Doctor> DoctorxEspxSede(int idSede, int idEsp);
    ...
    @Query(value = "SELECT * from doctor where idsede = ?1 LIMIT 5", nativeQuery = true)
    ...
    @Modifying
    @Query(value= "update doctor set formacion = ?1 , capacitaciones = ?2 where idusuario = ?3 ",nativeQuery = true)
    void actualizarDoctor (String formacion, String capacitaciones, Integer idUsuario);
    @Transactional
    @Modifying
